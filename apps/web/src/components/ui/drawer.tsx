  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div
        ...
        aria-modal="true"
        className={cn(
          "animate-drawer-in relative z-10 flex h-full w-full max-w-md flex-col border-l border-[--color-border] bg-white shadow-xl",
          className,
        )}
      >
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-[--color-border] px-6 py-4">
          <div className="min-w-0">
            <h2 className="text-lg font-semibold">{title}</h2>
            {description && <p className="mt-0.5 text-sm text-[--color-muted-foreground]">{description}</p>}
          </div>
          <button
            onClick={onClose}
            className="shrink-0 rounded p-1 transition-colors hover:bg-[--color-muted]"
            aria-label="Cerrar"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        {footer && <div className="shrink-0 border-t border-[--color-border] px-6 py-4">{footer}</div>}
      </div>
    </div>
  )
}
