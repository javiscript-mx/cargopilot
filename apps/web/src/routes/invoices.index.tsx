...
function InvoicesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["invoices", { page, search: debouncedSearch }],
    queryFn: () => invoicesApi.listPaged({ page, pageSize: PAGE_SIZE, search: debouncedSearch }),
                  ...
                  return (
                    <li key={inv.id} className="px-4 py-3">
                      <div className="flex items-center justify-between gap-3">
                        <span className="font-mono font-semibold">{inv.series}-{inv.folio}</span>
                        <Badge variant={status.variant}>{status.label}</Badge>
                      </div>
                      <p className="mt-1 truncate font-medium">{inv.customer.name}</p>
                      <p className="font-mono text-xs text-[--color-muted-foreground]">{inv.customer.rfc}</p>
                      <div className="mt-1 flex items-center justify-between gap-3">
                        <span className="text-xs text-[--color-muted-foreground]">{new Date(inv.createdAt).toLocaleDateString("es-MX")}</span>
                            ...
                            <>
                              ...
                              {isAdmin && (
                                <Button
                                  ...
                                  className="text-[--color-destructive] hover:bg-red-50"
