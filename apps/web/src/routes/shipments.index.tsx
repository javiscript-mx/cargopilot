...
function ShipmentsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["shipments", { page, pageSize, search: debouncedSearch }],
    queryFn: () => shipmentsApi.listPaged({ page, pageSize, search: debouncedSearch }),
    ...
    <AppLayout>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-[var(--color-muted-foreground)]">{total} operaciones en total</p>
        </div>
        {canWrite && (
            ...
            <>
              ...
              <ul className="divide-y divide-[var(--color-border)] md:hidden">
                {shipments.map((s) => {
                  ...
                  return (
                    <li key={s.id}>
                      <Link to="/shipments/$id" params={{ id: s.id }} className="block px-4 py-3 hover:bg-[var(--color-muted)]/50">
                        <div className="flex items-center justify-between gap-3">
                          <span className="font-mono font-semibold text-[var(--color-primary)]">{s.folio}</span>
              ...
              <thead>
                <tr className="border-b border-[var(--color-border)]">
                  ...
                  <th className="px-4 py-3 text-left font-medium text-[var(--color-muted-foreground)]">Operación</th>
                  <th className="hidden px-4 py-3 text-left font-medium text-[var(--color-muted-foreground)] md:table-cell">Ruta / Referencia</th>
                  <th className="px-4 py-3 text-left font-medium text-[var(--color-muted-foreground)]">Estado</th>
                  <th className="hidden px-4 py-3 text-left font-medium text-[var(--color-muted-foreground)] sm:table-cell">Fecha</th>
                </tr>
              </thead>
              <tbody>
                {shipments.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="border-b border-[var(--color-border)] last:border-0 hover:bg-[var(--color-muted)]/50">
                      <td className="px-4 py-3">
                        <Link to="/shipments/$id" params={{ id: s.id }} className="font-mono font-semibold text-[var(--color-primary)] hover:underline">
                          {s.folio}
                        </Link>
                      </td>
                      <td className="px-4 py-3">
                        <div className="font-medium">{s.customer.name}</div>
                        <div className="text-xs text-[var(--color-muted-foreground)]">{s.customer.rfc}</div>
                      </td>
                      ...
                      <td className="hidden px-4 py-3 md:table-cell">
                        {s.origin && s.destination ? (
                          ...
                          <span className="font-mono text-xs text-[var(--color-muted-foreground)]">{s.reference}</span>
                        ) : (
